...
class PaSST(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.forward_features(x)  # 12 x 472 x 768
...
def adapt_image_pos_embed_to_passt(posemb, num_tokens=1, posemb_len=1000, mode='bicubic'):
    ...
    # _logger.info('Resized position embedding: %s to %s with %s cls/dis tokens', posemb.shape,
    #              num_tokens)
    #
    # _logger.info('Position embedding grid-size from %s to %s', [gs_old, gs_old], gs_new)
    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k:
            ...
            # assert model.in_chans == 1, "More than 1 channel in the input image is not supported yet."
            # v = (v.transpose(1, 2)
            #      .reshape(-1, k2, W)
            #      .transpose(1, 2)
            #      .reshape(embed_dim, model.in_chans, k1, k2))  # [embed_dim, 1, k1, k2]
...
def get_model(arch="passt_s_kd_p16_128_ap486", pretrained=True, n_classes=527, in_channels=1,
              input_fdim=128, input_tdim=998, frame_patchout=300, pos_embed_length=1000, embed_dim=768, wandb_id=None
              ):
    """
